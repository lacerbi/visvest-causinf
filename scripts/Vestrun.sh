...
JOBSCRIPT="${BASEDIR}/matlab/scripts/fitdata.sh"
...
qsub -t ${JOBLIST} -v PROJECT=${PROJECT},MAXID=$MAXID,MAXRT=$MAXRT,WORKDIR=$WORKDIR,VERBOSE=${VERBOSE},NSAMPLES=${NSAMPLES} -l ${RESOURCES} -N ${JOBNAME} ${JOBSCRIPT}
