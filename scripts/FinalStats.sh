...
#Job parameters
RUN=${1}
WORKDIR="/scratch/la67/${PROJECT}/stats${RUN}"
mkdir ${WORKDIR}
cd ${WORKDIR}
MAXID=$(ls -l *.job | wc -l)
RUNTIME=24:00:00
...
qsub -t ${JOBLIST} -v PROJECT=${PROJECT},RUN=${RUN},MAXID=$MAXID,MAXRT=$MAXRT,WORKDIR=$WORKDIR,VERBOSE=${VERBOSE},NSAMPLES=${NSAMPLES},NBURNIN=${NBURNIN},STOREDSAMPLES=${STOREDSAMPLES},OPTFEVALS=${OPTFEVALS},CONTINUE=${CONTINUE},LOADMBAG=${LOADMBAG} -l ${RESOURCES} -N ${JOBNAME} ${JOBSCRIPT}
