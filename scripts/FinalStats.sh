...
qsub -t ${JOBLIST} -v PROJECT=${PROJECT},RUN=${RUN},MAXID=$MAXID,MAXRT=$MAXRT,WORKDIR=$WORKDIR,VERBOSE=${VERBOSE},NSAMPLES=${NSAMPLES},NBURNIN=${NBURNIN},OPTFEVALS=${OPTFEVALS},CONTINUE=${CONTINUE},LOADMBAG=${LOADMBAG} -l ${RESOURCES} -N ${JOBNAME} ${JOBSCRIPT}
