% VESTBMS_UNIMODALGENDATA Generate N new datasets from unimodal dataset X
...
%
function R = VestBMS_UnimodalLeftRightGendata(X,N,model,theta,priorinfo,bincenters,MAXRNG,XGRID,SSCALE)

% Program constants
if nargin < 9; XGRID = 201; end
if nargin < 10; SSCALE = 8; end

nTrialTypes = size(X, 1);
nTrialsPerType = sum(X, 2);

% Generate response probability matrix
[~,extras] = VestBMS_UnimodalLeftRightDatalike(X,model,theta,priorinfo,bincenters,MAXRNG,XGRID,SSCALE,[]);
