...
qsub -t ${JOBLIST} -v PROJECT=${PROJECT},RUN=${RUN},MAXID=$MAXID,MAXRT=$MAXRT,WORKDIR=$WORKDIR,VERBOSE=${VERBOSE},OPTFEVALS=${OPTFEVALS},NSAMPLES=${NSAMPLES},NBURNIN=${NBURNIN},CONTINUE=${CONTINUE},LOADMBAG=${LOADMBAG} -l ${RESOURCES} -N ${JOBNAME} ${JOBSCRIPT}
