...
%
function varargout = CueBMS_UnimodalDatalike(X,model,theta,priorinfo,MAXRNG,XGRID,SSCALE,sumover,randomize)
...
% xpdf = bsxfun(@rdivide, xpdf, sum(xpdf, 2)*dx);
xpdf = bsxfun(@rdivide, xpdf, simpson2(xpdf, 2)*dx);
...
if isinf(kappa)
    ...
    if wr == 0
        %responsepdf = sum(bsxfun(@times, xpdf, exp(-0.5*(bsxfun(@minus, sstar, X(:, 3))/sigmar).^2)), 2)*dx/sigmar/sqrt(2*pi);
        responsepdf = simpson2(bsxfun(@times, xpdf, exp(-0.5*(bsxfun(@minus, sstar, X(:, 3))/sigmar).^2)), 2)*dx/sigmar/sqrt(2*pi);
    else
        sigmartot = sqrt(sigmar^2 + wr^2*sstar.^2);
%        responsepdf = sum(bsxfun(@times, xpdf, ...
        responsepdf = simpson2(bsxfun(@times, xpdf, ...
            bsxfun(@rdivide, exp(-0.5*bsxfun(@rdivide, bsxfun(@minus, sstar, X(:, 3)), sigmartot).^2), sigmartot)), 2)*diff(xrange(1:2))/sqrt(2*pi);        
    end

else
    % Compute decision distribution for each x (power of the posterior)
    decisionpdf = log(postpdf)*kappa;
    decisionpdf = exp(bsxfun(@minus, decisionpdf, max(decisionpdf, [], 1)));
    decisionpdf = bsxfun(@rdivide, decisionpdf, qtrapz(decisionpdf, 1)*diff(srange(1:2)));
    
    % surf(decisionpdf); grid off
    
    rindex = floor(X(:, 3)*SSCALE) + MAXRNG*SSCALE + 1;
    rmod = X(:, 3)*SSCALE + MAXRNG*SSCALE + 1 - rindex;
    
%    responsepdf = sum(bsxfun(@rdivide, exp(-0.5*bsxfun(@rdivide, bsxfun(@minus, xrange, X(:, 2)), sigmas).^2), sigmas) .* ...
%        (bsxfun(@times, rmod, decisionpdf(rindex+1, :)) + bsxfun(@times, 1-rmod, decisionpdf(rindex, :))), 2);
    responsepdf = qtrapz(bsxfun(@rdivide, exp(-0.5*bsxfun(@rdivide, bsxfun(@minus, alpha*xrange + beta, X(:, 2)), sigmas).^2), sigmas) .* ...
        (bsxfun(@times, rmod, decisionpdf(rindex+1, :)) + bsxfun(@times, 1-rmod, decisionpdf(rindex, :))), 2)*diff(xrange(1:2))/sqrt(2*pi);
        
    % loglike = sum(log(responsepdf)) + nTrials*(log(diff(xrange(1:2))) - 0.5*log(2*pi));
...
if 0
    ...
    %normf = bsxfun_normcdf(xrange(end),S,sqrt(sigmas2)) - bsxfun_normcdf(xrange(1),S,sqrt(sigmas2));
    %normf(normf < 0.999)

    sigmasum2 = sigmar.^2 + bsxfun(@times,alphax.^2,sigmas2);    
    zeta = bsxfun_normpdf(bsxfun(@times,alphax,S), bsxfun(@minus,R, betax), sqrt(sigmasum2));
    mucdf = bsxfun(@plus, S.*sigmar.^2, bsxfun(@times,bsxfun(@times,alphax,bsxfun(@minus,R,betax)),sigmas2))./sigmasum2;
    sigmacdf = sqrt(bsxfun(@rdivide,sigmar.^2.*sigmas2,sigmasum2));
    responsepdf2 = sum(bsxfun(@times,zeta,bsxfun_normcdf(b,mucdf,sigmacdf) - bsxfun_normcdf(a,mucdf,sigmacdf)),2);    
    
    responsepdf - responsepdf2
