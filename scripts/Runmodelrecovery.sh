...
DATAFILENAME="VestBMS_fakedata_bimloc.mat"
...
qsub -t ${JOBLIST} -v PROJECT=${PROJECT},RUN=${RUN},MAXID=$MAXID,MAXRT=$MAXRT,WORKDIR=$WORKDIR,VERBOSE=${VERBOSE},OPTFEVALS=${OPTFEVALS},NSAMPLES=${NSAMPLES},NBURNIN=${NBURNIN},STOREDSAMPLES=${STOREDSAMPLES},NEWSAMPLING=${NEWSAMPLING},CONTINUE=${CONTINUE},LOADMBAG=${LOADMBAG},DATAFILENAME=${DATAFILENAME} -l ${RESOURCES} -N ${JOBNAME} ${JOBSCRIPT}
